...
<p>
The LabelMe Matlab toolbox is designed to allow you to download and
interact with the images and annotations in the LabelMe database.  The
toolbox contains functions for plotting and querying the annotations,
computing statistics, dealing with synonyms, etc.  This page gives a
step-by-step overview of the main toolbox functionalities.
</p>
...
<h3>1. <a href="https://github.com/CSAILVision/LabelMeToolbox">Github repository</a></h3>

<p>We maintain the latest version of the toolbox on github. To pull the latest version, make sure that "git" is installed on your machine and then run "git clone https://github.com/CSAILVision/LabelMeToolbox.git" on the command line. You can refresh your copy to the latest version by running "git pull" from inside the project directory.</p>

<h3>2. <a href="https://github.com/CSAILVision/LabelMeToolbox/archive/master.zip">Zip file</a></h3>

<p>The zip file is a snapshot of the latest source code on github.</p>

<h2>Citation</h2>

<p>If you use this dataset, the annotation tool, or the functions on this toolbox, we would appreciate if you cite:</p>

<p>B. C. Russell, A. Torralba, K. P. Murphy, W. T. Freeman,
<br />LabelMe: a database and web-based tool for image annotation.
<br />International Journal of Computer Vision, pages 157-173, Volume 77, Numbers 1-3, May, 2008.
(<a href="http://people.csail.mit.edu/brussell/research/AIM-2005-025-new.pdf">paper.pdf</a>)
</p>


<h3>To read more about the difficulties of image annotation:</h3>

<p>A. Barriuso and A. Torralba.
<br />Notes on image annotation.
<br /><a href="http://arxiv.org/abs/1210.3448">arXiv:1210.3448 [cs.CV]</a> October, 2012.
(<a href="http://people.csail.mit.edu/torralba/publications/memories.pdf">paper.pdf</a>)
</p>
...
<pre>D = LMdatabase(HOMEANNOTATIONS);</pre>

<p>D is an array with as many entries as there are annotated images. For image n, some of the fields are:</p>
...
<p>Once you have created the LabelMe index D, you can visualize the annotations for one image with the function LMplot:</p>
...
<h3>Object names and attributes</h3>

<p>For each object, we store the object name in the field:</p>
<pre>
D(n).annotation.object(m).name
</pre>
<p>and the object attributes</p>
<pre>
...
<p>Part relationships are stored as a tree. If parts are annotated, the object will have two fields</p>
<pre>
...
<p> ispartof: contains the id of the parent object.</p>
<p> hasparts: comma separated list of the ids of the polygons that are parts of the current object.</p>

<p>Both fields provide redundant information, but there are there to simplify exploring the part tree. For instance, if <em>ispartof</em> field is empty, this indicates that the current object is not a part.</p>

<h3>Collecting annotation statistics</h3>

<p>The database contains many different object names. In order to see the list of object names and the number of times each object appears, you can use the function LMobjectnames which shows the distribution of object names when there are no output arguments:</p>

<pre>LMobjectnames(D);</pre>

<p>You can also get the list of object names and counts:

<pre>[names, counts] = LMobjectnames(D);</pre>

<p>You can also get the list of attributes:</p>
<pre>LMobjectnames(D, 'attributes');</pre>


<h3>Queries</h3>

<p>To perfom searches for images, scenes, objects, attributes, etc, you can use the function LMquery. This function allows searching for the content of any field.</p>
...
<p>For instance, if you want to search for all the objects annotated with the attribute 'red', you can user LMquery:</p>

<pre>Dred = LMquery(D, 'object.attributes', 'red', 'word');</pre>

<p>Note the fourth argument 'word'. The fourth argument specifies the method used for matching. There are three methods (help LMquery will give you the full description):</p>
...
<h3>Dealing with synonyms and labeling noise</h3>
